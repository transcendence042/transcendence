    ...
    return (
        <form onSubmit={handleRegister} className="flex flex-col">
            ...
            <div className="mt-3">
                ...
                <input name="email" type="email" className="w-full border-2 p-3 rounded-md mb-4" placeholder="Email"/>
                <h2 className="py-2">Display Name (optional):</h2>
                <input name="displayName" type="text" className="w-full border-2 p-3 rounded-md mb-4" placeholder="DisplayName"/>
...
const LoginForm: React.FC<LoginFormProps> = ({ setToken, setLoading }) => {
    ...
    const handleLogin = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        try {
            ...
            // *** FIX CRÍTICO: SIEMPRE quitar loading state ***
            setLoading(false);
        }
    };

    return (
        <div className="flex flex-col"> {/* CAMBIO: Fragment <> -> <div> */}
...
const Login: React.FC = () => {
    const authContext = useContext(AuthContext) as AuthContextType | undefined; // CAMBIO: Tipar useContext
    ...
    const { setToken, setLoading } = authContext;
    
    // Verificar que las funciones existen (por las propiedades opcionales en AuthContextType)
